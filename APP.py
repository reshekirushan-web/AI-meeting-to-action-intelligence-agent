...
if not st.session_state.authenticated:
    ...
    with st.container(border=True):
        ...
        if st.button(
            ...
            # MULTI-USER LOGIN
            # ----------------------------------------------------
            if username == "vimal" and password == "vimal123":
                st.session_state.authenticated = True
                st.session_state.logged_in_user = "vimal"
                st.session_state.page = 1
                st.rerun()

            elif username == "arun kumar" and password == "arun123":
                st.session_state.authenticated = True
                st.session_state.logged_in_user = "arun kumar"
                st.session_state.page = 1
                st.rerun()

            elif username == "reshe" and password == "jandaa":
                st.session_state.authenticated = True
                st.session_state.logged_in_user = "reshe"
                st.session_state.page = 1
                st.rerun()

            elif username == "hari" and password == "thudippu":
                st.session_state.authenticated = True
                st.session_state.logged_in_user = "hari"
                st.session_state.page = 1
                st.rerun()
...
# ============================================================
    ...
    st.info(
        "🛡️ Open-set recognition is enabled: if a person in the meeting "
        "is not in the reference list, Gemini must label that person as "
        "**Unknown** instead of forcing a registered-name match."
    )
...
IMPORTANT OPEN-SET VOICE-MATCHING RULES:
- Each reference audio belongs ONLY to the registered speaker named immediately before it.
- The meeting may contain people who are NOT present in the reference list.
- "Unknown" is a valid and expected result — it is NOT an error.
- NEVER force a meeting speaker to match one of the registered names just because that name is the closest available candidate.
- First decide whether the speaker matches ANY registered reference. If none is sufficiently strong, return "Unknown".
...
- A meeting speaker can be matched to a registered speaker only when the acoustic evidence is reasonably strong.
- Short, noisy, distorted, overlapping, or unclear speech should normally be returned as "Unknown".
- Do not invent identities.
- One registered person should normally map to at most one distinct meeting speaker.
- If multiple registered references are similar, use "Unknown" when the evidence cannot reliably separate them.
- Give a confidence value from 0 to 1.
- For "Unknown", confidence means confidence that NO registered reference is a reliable match.
- Use the exact string "Unknown" for an unregistered speaker.
...
Rules:
- "Unknown" is a first-class speaker identity, not an error.
- If a meeting speaker is not in the reference list, ALWAYS use "Unknown".
- Never substitute the closest registered name for an unknown speaker.
- Use "Unknown" whenever acoustic evidence is weak, conflicting, too short, noisy, overlapped, or otherwise insufficient.
...
if st.session_state.meeting_data:
    ...
    for index, item in enumerate(
        ...
        if str(name).strip().casefold() in {"unknown", "unknown speaker", "unregistered"}:
            st.warning(
                f"🎤 {label} → **Unknown (not in reference list)** "
                f"(confidence: {confidence_text})"
            )
            st.caption(
                "ℹ️ This speaker was not reliably matched to any registered reference voice."
            )
        else:
            st.success(
                f"🎤 {label} → **{name}** "
    ...
    report.append(
        "NOTE: 'Unknown' means the meeting speaker was not reliably matched "
        "to any registered reference voice."
    )
    report.append("")
